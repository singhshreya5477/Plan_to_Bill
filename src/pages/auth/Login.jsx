  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.email && formData.password) {
      ...
      const userData = {
        ...
        role: formData.email.includes('admin') ? 'admin' : 'project_manager',
        company: 'Plan-to-Bill'
      };
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden" 
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full opacity-30 animate-float gradient-bg-purple"
          style={{ filter: 'blur(80px)' }}
        />
        <div className="absolute bottom-0 right-0 w-80 h-80 rounded-full opacity-30 gradient-bg-pink"
          style={{ 
            filter: 'blur(70px)',
            animation: 'float 8s ease-in-out infinite reverse'
          }}
        />
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full opacity-25 gradient-bg-blue"
          style={{ 
            filter: 'blur(60px)',
            animation: 'float 10s ease-in-out infinite'
          }}
        />
        <div className="absolute top-1/4 right-1/4 w-72 h-72 rounded-full opacity-20 gradient-bg-teal"
          style={{ 
            filter: 'blur(65px)',
      ...
      <div className="w-full lg:w-1/2 relative z-10 flex items-center justify-center p-6 lg:p-12">
        <div className="max-w-md w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button 
              type="submit" 
              className="w-full btn-primary py-3 font-medium flex items-center justify-center group"
            >
              <span>Sign In</span>
